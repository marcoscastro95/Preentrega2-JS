...
const generarMenuDeherramientas = (listaDeHerramientas) => {
    let mensajeDelMenu = 'Menu:';
    let opcion = 0;
    const copiaDeLista = listaDeHerramientas;
    copiaDeLista.forEach(herramienta => {
        opcion++;
        mensajeDelMenu += '\nOpcion ' + opcion + ': '+ herramienta.mostrarInfo();
    })
...
const precioUnidad = 50;
const descuentoMediaDocena = 0.10;
const descuentoDocena = 0.15;
const cantidadDeOpciones = 6;

const listaherramientas = [
    new destornillador('Phillips C', 'PH'),
    new destornillador('Phillips G', 'PH'),
    new destornillador('Paleta C', 'PA'),
    new destornillador('Paleta G', 'PA'),
    new destornillador('Torx C', 'TO'),
    new destornillador('Torx G', 'TO'),
];

const promocionesBancarias = [
    new PromoBancaria('Banco Brou', 0.15),
    new PromoBancaria('Banco Itau', 0.05),
    new PromoBancaria('Banco Bandes', 0.05),
    new PromoBancaria('Scotiabank', 0.10),
];

const mensajeBienvenida = `¡Bienvenido a Destornilladores Online!
...
Elegí a continuación la cantidad y las herramientas.`;

const mensajeMenu = generarMenuDeherramientas(listaherramientas);

const mensajeSolicitardestornillador = `${mensajeMenu}
...
const ingresarCantidad = () => {
    const cantidad = Number(parseInt(prompt('Ingresa la cantidad de Destornilladores que vas a llevar:')));
    return cantidad;
};
...
const mostrarPedidoEnConsola = (pedido) => {
    let mensaje = 'Los herramientas elegidas son:';
    pedido.forEach(item => {
        mensaje += '\n- ' + item;
    })
    console.log(mensaje);
...
const agregarOpcionAlPedido = (opcion) => {
    ...
    pedidoUsuario.push(listaherramientas[indice].nombre);
};

const solicitarherramientas = (cantidad) => {
    for (let i = 0; i < cantidad; i++) {
        let opcion = solicitarOpcion(mensajeSolicitardestornillador, cantidadDeOpciones);
